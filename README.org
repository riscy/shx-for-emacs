...
* Description
  ...
  /This version is tested with Emacs 25.1.1/  Check out the [[#changelog][changelog]].
* Related
...
* Install
  Installing via MELPA is on its way.

  For now, add the following to your ~.emacs~:
  #+begin_src elisp
...
* Changelog
*** 0.0.1
    To make shx a better fit for MELPA, I've modified it for compatibility with
    other packages.  Once it's on MELPA it will follow semantic versioning.
    Breaking changes are as follows:
    1. Obsolete function aliases have been removed, so the following commands
       will no longer work out of the box: ~barplot~, ~matrix~, ~plot~, and
       ~scatter~. Use ~plotbar~, ~plotmatrix~, ~plotline~, and ~plotscatter~
       instead.
    2. The URL-browsing binding ~C-c C-b~ has been retired (it clashed with an
       existing shell-mode binding).  Instead you can use the shx user command
       ~:go~, or click on a visible URL, or arrow up to a URL and press
       ~<enter>~.
    3. ~shx-split.el~ has moved: https://github.com/riscy/split-scroll
