...
*** Customize
    ...
    (setq
      ...
      shx-max-input 1024
      ;; prefer "/bin/sh" when opening a shell on Docker containers
      shx-customized-shell-commands '(("/docker:.*" . "/bin/sh"))
...
* Extra shell commands
  ...
  :ssh
  #+end_src
  Thanks to input from [[https://github.com/p3r7][Jordan Besly]], you can customize the default interpreter
  for each "remote" by customizing ~shx-customized-shell-commands~.
