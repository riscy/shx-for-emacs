...
* Description
  ...
  Adds (and makes it easy to add) new shell commands written in elisp:
  ...
  Use ~M-x shx <return>~ to begin a new ~shell~ session with ~shx~ enabled.
...
*** Manually
    ...
    #+end_src
* Setup
  Type ~M-x shx <return>~.  The following commands will get you started:
  1. Type ~:man ls~ to have the man page for ~ls~ brought up in another window.
